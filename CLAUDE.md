...
- **Branch 3:** Gemini native function-calling orchestrator (`src/adapters/llm/orchestrator.ts`). Manager can use tools conversationally: calendar read/write/delete, web search, customer lookup, contact notes, business configuration. Deterministic apply pipeline enforced inside `manageBusinessSettings` tool. Full session memory (4h expiry) + last 3 cross-session summaries. See `MULTI_AGENT_DESIGN.md` and `CHAT_LEVEL_LAWBOOK.md`.
  - **Meeting coordination (owner-only, outbound sub-case of Branch 3 — not a fifth branch):** the owner can have the PA reach out to an external person and negotiate a meeting time. `coordinateMeeting` / `resolveMeetingCoordination` tools drive a `meeting_coordinations` state machine (`src/domain/coordination/`); the PA negotiates within owner-given fallback times and books only on a final owner confirm. Counterparties are a new `contact` identity role (excluded from the CRM); their replies route via `routeContactMessage` (`src/routes/webhook.ts`). Gated by the `meeting.coordinate` authorization action — managers always, delegated users only if granted, customers/contacts never. Stale coordinations expire via the `coordination-expiry` worker (`COORDINATION_EXPIRY_HOURS`, default 72). Design: `docs/superpowers/specs/2026-06-21-meeting-coordination-design.md`.
...
- **Two gender axes (don't conflate):** `botPersona` = how the PA refers to **itself** (the PA's self-voice, a business-level setting); `identities.addresseeGender` = how the PA addresses **the person** in Hebrew 2nd-person (masculine/feminine/unknown). Addressee gender is resolved pre-LLM (sibling of language) and applied at the one chokepoint `buildVoiceCore(channel, addresseeGender)`; unknown → masculine floor, never split-gender. See `CHAT_LEVEL_LAWBOOK.md §3.5`.
