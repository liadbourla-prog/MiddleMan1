...
- **Three doors** — every seam that reaches `sendMessage` is gated, but not identically: **Branch 4** `makeGenReply` (`customer-booking.ts`) and **Branch 3** `gateAndAuditBranch3Reply` (`orchestrator.ts`) both run the full `gateReply` (booking/time/occupancy/action + Gate 4 + regen cap + voice monitor); the **proactive** door (`generateProactiveCustomerMessage` → `gateProactiveBody`, `client.ts`) runs a **structurally-analogous but lighter subset** — `detectActionClaims` (enforced where `backedActions` is supplied) + an opt-in time check + **Gate-4 `hasActionFabrication` check/ask enforcement** + the **`observeVoiceTells` mechanical monitor** (both added in P3-C1) — but with **no** booking/occupancy gate and **no** regen. (Locked design D3: workers supply no per-turn ledger, so the proactive door's job is the action/check-ask enforcement + the structural chokepoint; waitlist time-truthfulness is enforced by fresh-spine re-validation before send, not by the gate.) A cross-seam non-bypass invariant test (`grounding/cross-seam-non-bypass.test.ts`) proves no reply reaches send without traversing its door's gate.
