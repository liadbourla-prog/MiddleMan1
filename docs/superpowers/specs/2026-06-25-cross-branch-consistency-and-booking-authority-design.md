...
- ✅ **INV-3 proactive owner-notification.** New spine initiator `booking.new_for_owner` (Layer B, owner audience, transactional, window-ungated); `notifyOwnerNewBooking` fired from the immediate-gate self-book confirm paths, gated by the `new_booking` notification rule (defaults to notify; owner can mute via `configureNotifications`). Deliberately **not** fired on the post-payment finalize path (§10 keeps the autonomous payment loop owner-silent). Unit-tested.

**Remaining (follow-up — scoped, not yet built):**
1. **Owner reflection for post_payment bookings** — currently carved out to preserve the §10 owner-silent payment loop; decide whether a muted-by-default reflection belongs at the booking-request (pending_payment) moment instead.
